...
Overall, Alliance performance improved over the last month, increasing it's level of performance above and beyond the measures, Follow Up After Hospitalization (FUH 7 Day), Anti-depressant Medication Management (AMM2), and Use of Opioids at High Dosage (HDO).
...
VBP_Rep_Comb_validation_complete <- read_csv("./data/VBPQR_AllApsCombined_Cleaned_complete_2023-07-21.csv")
...
Data.Period = "2023-06-30" #claimsAdjudicated Through Date
Reportasof = "2023-07-21" #Report as of date from VBP Quality report
ProgressReportDate = "2023-07-31" # Data that the VBPQR was released, or represents, usually the 1st of the month for the Progress Report
Members_Report_Date = "2023-07-26" #Date that the global members data was extracted

current_date <- "2022-07-26"
...
# Alliance Membership Demographic Information

```{r}
#| label: Create_ActiveNAZMembership_DataSet
#| include: true
#| echo: true
#| warning: false
#| error: false
...
# Calculate Age from DOB and add new variable to ActiveNAZMembership
ActiveNAZMmbrshp$dob <- as.Date(ActiveNAZMmbrshp$dob)
ActiveNAZMmbrshp$Age <- as.integer(difftime(current_date, ActiveNAZMmbrshp$dob, units = "days")/365.25)

# Export Active NAZ Membership to csv, update date to date of Progress Report
write.csv(ActiveNAZMmbrshp, "./data/output/2023-07-26_ActiveNAZMembership.csv")
write.csv(ActiveNAZMmbrshp, "C:/Users/KGLtd/OneDrive - The NARBHA Institute/Documents - Data Force/Projects/AllianceIntranetSupport/data/ActiveNAZMembership.csv")
```
...
write.csv(CountyDistribution, "./data/output/2023-07-26_CountyDistribution.csv")

# Create objects for inline code
max_cnty_percent <- max(CountyDistribution$Percent)
max_cnty_name <- CountyDistribution$`County of Residence`[CountyDistribution$Percent == max_cnty_percent]

min_cnty_percent <- min(CountyDistribution$Percent)
min_cnty_name <- CountyDistribution$`County of Residence`[CountyDistribution$Percent == min_cnty_percent]
```
...
# Report Output
gtsave(Tab_CountyDistribution, filename = "./data/output/2023-07-26_Tab_CountyDistribution.png")
gtsave(Tab_CountyDistribution, filename = "./data/output/2023-07-26_Tab_CountyDistribution.png")

# Dashboard
gtsave(Tab_CountyDistribution, filename = "C:/Users/KGLtd/OneDrive - The NARBHA Institute/Site Assets/SitePages/Alliance-Dashboard/2023-07-26_Tab_CountyDistribution.png")
...
Tab_Gender <- ActiveNAZMmbrshp |> 
  ...
  mutate(Percent = (Members / (sum(Members)))) 
# Export the table as a csv for reference, Date is date of progress report
write.csv(Tab_Gender, "./data/output/2023-07-26_Tab_Gender.csv")
```

The Alliance serves adults and children aged 6 and older. Distribution of gender is balanced, with Male accounting for `r Tab_Gender |> filter(Sex == "Male") |> select(Percent)`, and Female accounting for `r Tab_Gender |> filter(Sex == "Female") |> select(Percent)`. There is no change over last month.
:::
...
gt() |>
  ...
  #            Members ~ px(125),
  #            Percent ~ px(125)) |> 
  tab_source_note(source_note = md("*SOURCE: BCBSAZ Global Members, extracted 07-26-2023*")) |> 
  tab_style(
    style = list(
      cell_fill(color = "#31688E"),
      cell_text(weight = "bold",
...
# Report Output
gtsave(Tab_GenderDistribution, filename = "./data/output/2023-07-26_Tab_GenderDistribution.png")
gtsave(Tab_GenderDistribution, filename = "./data/output/2023-07-26_Tab_GenderDistribution.png")

# Dashboard
gtsave(Tab_GenderDistribution, filename = "C:/Users/KGLtd/OneDrive - The NARBHA Institute/Site Assets/SitePages/Alliance-Dashboard/2023-07-26_Tab_GenderDistribution.png")
...
Fig_PopPyramid <- ggplot(DemoTable_SexByAgeGroup, aes(y = Age_Group, x = n, fill = sex)) +
   ...
   geom_text(aes(label = abs(n), 
                 hjust = ifelse(
                   sex == "Female", -.3, 1.3)),
                 size = 3) +
   scale_fill_manual(values = c(Female = "#31688E", Male = "#440154"), drop = FALSE) +
...
Dashboard_FileName1 <- "2023-07-26_Fig_PopPyramid.png"
Dashboard_FileName2 <- "Fig_PopPyramid.png"
...
#Dashboard
ggsave(file.path(Dashboard, Dashboard_FileName1), plot = Fig_PopPyramid, width = 6, height = 4, dpi = 300)
...
As the COVID19 pandemic public health emergency (PHE), and the resulting restrictions on disenrollment of Medicaid recipients, is ended, we will monitor the rates of enrollment. It is estimated that 37,000 enrolled BCBSAZ members will lose coverage when the PHE ends. It is unknown how many of those will be Alliance members.
...
write.csv(ContractAlloc_v_Assignment, "./data/output/2023-07-26_ContractAlloc_v_Assignment.csv")
...
# Report Output
gtsave(Tab_Alloc_v_Assignment, filename = "./data/output/2023-07-26_Tab_Alloc_v_Assignment.png")
gtsave(Tab_Alloc_v_Assignment, filename = "./data/output/2023-07-26_Tab_Alloc_v_Assignment.png")

# Dashboard
gtsave(Tab_Alloc_v_Assignment, filename = "C:/Users/KGLtd/OneDrive - The NARBHA Institute/Site Assets/SitePages/Alliance-Dashboard/2023-07-26_Tab_Alloc_v_Assignment.png")
...
# Export the table as a csv for reference, date is date of progress report
write.csv(Tab_PercentCompliant, "./data/output/2023-07-26_Tab_PercentCompliant.csv")
...
gt() |> 
  tab_header(
    ...
    subtitle = "Report as of 2023-07-26"
  ) |> 
  ...
  cols_label(SubMeasureID = "NCQA Measure",
   ...
   #           ) |>
  tab_source_note(
    source_note = md("*SOURCE: BCBSAZ VBP Performance Reports, Claims Adjudicated Through 2023-06-30* </br> *FUM not required by contract*")
  ) |> 
  tab_style(
    style = list(
      cell_fill(color = "#31688E"),
...
# Report Output
gtsave(Tab_AllProvPerformance, filename = "./data/output/2023-07-26_Tab_AllProvPerformance.png")
gtsave(Tab_AllProvPerformance, filename = "./data/output/2023-07-26_Tab_AllProvPerformance.png")

# Dashboard
gtsave(Tab_AllProvPerformance, filename = "C:/Users/KGLtd/OneDrive - The NARBHA Institute/Site Assets/SitePages/Alliance-Dashboard/2023-07-26_Tab_AllProvPerformance.png")
...
# 
# write.csv(Tab_AllProvLollipop, "./data/output/2023-07-26_Tab_AllProviderLollipop.csv")
# 
...
# # Set the file path and name for saving the .png file
# Report <- "./data/output/"
# Report_Filename <- "2023-07-26_Fig_AllProvLollipop.png"
# 
# Dashboard <- "C:/Users/KGLtd/OneDrive - The NARBHA Institute/Site Assets/SitePages/Alliance-Dashboard/"
# Dashboard_FileName1 <- "2023-07-26_Fig_AllProvLollipop.png"
...
write.csv(Tab_AllProvLollipop, file = "./data/output/2023-07-26_Tab_AllProviderLollipop.csv")
...
# Save the ggplot as a .png file
ggsave(file = "./data/output/2023-07-26_Fig_AllProvLollipop.png", plot = Fig_AllProvLollipop, width = 6, height = 4, dpi = 300)
...
write.csv(IndivAlliancePerformance_AMM2, "./data/output/2023-07-26_IndivAlliancePerformance_AMM2.csv")
```
...
gt() |> 
  ...
  cols_label(
   ...
   #           ) |>
  tab_source_note(
    source_note = md("*SOURCE: BCBSAZ VBP Quality Roster, Claims Adjudicated Through 2023-06-30*")
  ) |> 
  tab_style(
    style = list(
      cell_fill(color = "#31688E"),
...
# Report Output
gtsave(Tab_IndivAlliancePerformance_AMM2, filename = "./data/output/2023-07-26_Tab_IndivAlliancePerformance_AMM2.png")
gtsave(Tab_IndivAlliancePerformance_AMM2, filename = "./data/output/2023-07-26_Tab_IndivAlliancePerformance_AMM2.png")

# Dashboard
gtsave(Tab_IndivAlliancePerformance_AMM2, filename = "C:/Users/KGLtd/OneDrive - The NARBHA Institute/Site Assets/SitePages/Alliance-Dashboard/2023-07-26_Tab_IndivAlliancePerformance_AMM2.png")
...
write.csv(IndivAlliancePerformance_FUH7, "./data/output/2023-07-26_IndivAlliancePerformance_FUH7.csv")
```
...
# Report Output
gtsave(Tab_IndivAlliancePerformance_FUH7, filename = "./data/output/2023-07-26_Tab_IndivAlliancePerformance_FUH7.png")
gtsave(Tab_IndivAlliancePerformance_FUH7, filename = "./data/output/2023-07-26_Tab_IndivAlliancePerformance_FUH7.png")

# Dashboard
gtsave(Tab_IndivAlliancePerformance_FUH7, filename = "C:/Users/KGLtd/OneDrive - The NARBHA Institute/Site Assets/SitePages/Alliance-Dashboard/2023-07-26_Tab_IndivAlliancePerformance_FUH7.png")
...
write.csv(IndivAlliancePerformance_HDO, "./data/output/2023-07-26_IndivAlliancePerformance_HDO.csv")
```
...
# Report Output
gtsave(Tab_IndivAlliancePerformance_HDO, filename = "./data/output/2023-07-26_Tab_IndivAlliancePerformance_HDO.png")
gtsave(Tab_IndivAlliancePerformance_HDO, filename = "./data/output/2023-07-26_Tab_IndivAlliancePerformance_HDO.png")

# Dashboard
gtsave(Tab_IndivAlliancePerformance_HDO, filename = "C:/Users/KGLtd/OneDrive - The NARBHA Institute/Site Assets/SitePages/Alliance-Dashboard/2023-07-26_Tab_IndivAlliancePerformance_HDO.png")
